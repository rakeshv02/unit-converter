...
export default function UnitConverter() {
  ...
  const S = {
    ...
    swapBtn:    { width: 40, height: 40, borderRadius: '50%', background: '#f8f7f4', border: '1.5px solid #e2e8f0', cursor: 'pointer', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, alignSelf: 'center', marginTop: 20 },
    resultRow:  { background: '#f8f7f4', borderRadius: 10, padding: '12px 16px', fontSize: 14, color: '#475569', textAlign: 'center', marginBottom: 12 },
  ...
  const catTabStyle   = (key) => ({ padding: '6px 14px', borderRadius: 16, border: 'none', background: category === key ? groupColor : '#f8f7f4', color: category === key ? 'white' : '#64748b', fontWeight: 600, fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 });
          ...
          <div style={S.ioRow}>
            ...
            <div>
              ...
              <div style={S.outputBox}>
                {output !== '' ? String(output) : <span style={{ color: '#e2e8f0' }}>Result</span>}
              </div>
            </div>
          </div>
          {output !== '' && input !== '' && (
